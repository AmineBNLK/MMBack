...
exports.getTerrains = async (req, res, next) => {
  try {
    const terrains = await Terrain.find().populate("joueur")
...
// @access  Private
exports.createTerrain = asyncHandler(async (req, res, next) => {
  ...
  // const terrain = await Terrain.create({ joueur: req.body.joueur });
  //////////////////////////////////////////

  Terrain.create(req.body).then((terrain) => {
    res.status(201).json({
      success: true,
      data: terrain,
    })
...
//   req.body.joueur = req.joueur.id; // Ajouter le joueur associé au terrain
...
//     });
//   } catch (error) {
//     return next(new ErrorResponse('Unable to create terrain', 500));
//   }
// });
...
exports.terrainPhotoUpload = asyncHandler(async (req, res, next) => {
  ...
  file.name = `photo_${terrain._id}${path.parse(file.name).ext}`
  ...
  file.mv(`${process.env.FILE_UPLOAD_PATH}/${file.name}`, async (err) => {
    ...
    await Terrain.findByIdAndUpdate(req.params.id, { image: file.name })
    res.status(200).json({
      success: true,
      data: file.name,
    })
