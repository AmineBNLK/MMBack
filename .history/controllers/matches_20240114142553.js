...
exports.createMatch = asyncHandler(async (req, res, next) => {
  ...
  const lastMatch = Match.findOne({ joueur: req.joueur.id })
    .sort({
      date: -1,
    })
    .then((match) => {
      // Date 1
      const date1 = new Date('2024-01-14T12:00:00');
      ...
      // Vérification si la différence est égale à 24 heures
      if (differenceInHours === 24) {
        console.log('La différence est de 24 heures.');
      } else {
        console.log("La différence n'est pas de 24 heures.");
      }
    });
