...
JoueurSchema.methods.supprimerAmiMutuel = async function(amiId) {
  // Supprimer amiId de la liste d'amis du joueur courant
  this.amis = this.amis.filter((ami) => ami.toString() !== amiId);

  // Trouver l'ami dans la base de données
  const ami = await this.model('Joueur').findById(amiId);

  if (!ami) {
    throw new Error("Ami non trouvé.");
  }

  // Supprimer l'ID du joueur courant de la liste d'amis de l'ami
  ami.amis = ami.amis.filter((ami) => ami.toString() !== this._id.toString());

  // Sauvegarder les modifications
  await Promise.all([this.save(), ami.save()]);
};


// Ajoutez la méthode supprimerAmiPermanent
JoueurSchema.methods.supprimerAmiPermanent = async function (amiId) {
