...
exports.getJoueurs = asyncHandler(async (req, res, next) => {
  const joueur = await Joueur.findById(req.params.id)
    .populate('amis', 'name email') // Remplace les identifiants dans amis par les documents réels
    .populate('tempAmis', 'name email'); // Remplace les identifiants dans tempAmis par les documents réels
